...
function BarChart() {
  const [userData, setUserData] = useState({ doctors: Array(12).fill(0), users: Array(12).fill(0), nurses: Array(12).fill(0) });

  useEffect(() => {
    fetch('http://localhost:8000/api/user/')
      .then(response => response.json())
      .then(data => {
        const roleCount = { doctors: Array(12).fill(0), users: Array(12).fill(0), nurses: Array(12).fill(0) };
        data.forEach(user => {
